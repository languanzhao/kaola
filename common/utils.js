...
export function login_mp_wx(App,paramsCode){
	...
	const that = context
	if(store.state.userInfo.id){
		console.log(store.state.userInfo,'我已经登录')
		//用户登录了
		that.$http({
			api: api.getUserInfo,
			method: 'GET'
		...
		uni.getUserInfo({
		  success: res => {
			...
			context.$http({
				...
				context.$store.commit('setUserInfo',info.data)
			}).catch(err => {
...
export function login_wx_h5(App,type,paramsCode){
			const that = context
				...
				if(!that.$store.state.userInfo.id){
					...
					that.$http({
						...
						that.$store.commit('setUserInfo', res.data)
							...
							console.log('用户登录了' + codeParam)
							// uni.showToast({
							// 	icon:'none',
							// 	title:'用户登录了' + codeParam
							// })
							//锁粉
							if(codeParam){
								if(!is_counselor && !is_registration && !parent_id){
...
//设置用户信息  
 export function set_user_info(data,type,paramsCode){
			const that = context
			//保存token
			let token = {
