<template>
	<view class="detail">
		...
		<view class="u-skeleton">
			...
			<!-- <u-sticky>
					<view class="detail_tab">
						<view :class="['tab-item','u-skeleton-rect',current === index ? 'currentIcon' : '']" v-for="(item,index) of tabList" :key='index' @click="tabClick(index)">
							<text :class="['iconfont',current === index ? item.currentIcon : item.icon]"></text>
							<text class="title">{{item.title}}</text>
						</view>
					</view>
				...
				return new Promise((resolve, reject) => {
					...
					this.$http({
						...
						params: {
							page: 1,
							perPage: 10,
							'filter[school_id]': this.schoolId
						}
					}).then(res => {
